...
proof.
  ...
  + proc => /=.
    inline ChaCha20_pref.M.update_ptr;wp.
    do 2! unroll for ^while;wp;skip => /> &hr (* hinv *) hlen j.
    rewrite /x4_ /=.
    have hmem1 := storeW256_upd_mem Glob.mem{hr} Glob.mem{hr} [] output{hr} plain{hr} k1 _; 1: by smt().
    have {hmem1} hmem2 := storeW256_upd_mem Glob.mem{hr} _ _ output{hr} plain{hr} k2 hmem1.
    have {hmem2} hmem3 := storeW256_upd_mem Glob.mem{hr} _ _ output{hr} plain{hr} k3 hmem2.
    by apply (storeW256_upd_mem Glob.mem{hr} _ _ output{hr} plain{hr} k4 hmem3).
  ...
  transitivity Store_x4.store_x4 
    (={Glob.mem, output, plain, len} /\ k{2} = (x4_ k_1 k_2 k_3 k_4){1} /\ 256 <= len{1} /\
     ...
     ==> 
      ...
      (good_ptr res{1}.`1 res{1}.`2 res{1}.`3){1} /\ (inv_ptr res{1}.`1 res{1}.`2 res{1}.`3){1})
    (={Glob.mem, k} /\ 
       output{1} = to_uint output{2} /\ plain{1} = to_uint plain{2} /\ len{1} = to_uint len{2} /\ 
       (good_ptr output plain len){1} /\ (inv_ptr output plain len){1} /\ 256 <= len{1} ==>
     res{1}.`1 = to_uint res{2}.`1 /\ res{1}.`2 = to_uint res{2}.`2 /\ res{1}.`3 = to_uint res{2}.`3 /\ 
     ={Glob.mem}).
  + by move=> /> *; exists Glob.mem{2} (output, plain, len, x4_ k_1 k_2 k_3 k_4){1}; rewrite H6 => />.
  + done. 
  + proc *.
...
proof.
  ...
  + by move=> *;rewrite (Array8.ext_eq_all sk).
