...
require import ChaCha20_Spec ChaCha20_pref ChaCha20_pref_proof ChaCha20_pavx2_cf (* ChaCha20_pavx2 *).
...
phoare chacha20_body_ll : [ChaCha20_pavx2_cf.M.chacha20_body : true ==> true] = 1%r.
proof.
...
equiv pref_pavx2_cf_less_than_128: 
  ChaCha20_pavx2_cf.M.chacha20_ref_loop ~ ChaCha20_pavx2_cf.M.chacha20_less_than_128 :
  ={Glob.mem, output, plain, len, st_1} /\ (0 <= len <= 128){1}
  ==>
  ={Glob.mem}.
proof.
  proc => /=; inline ChaCha20_pavx2_cf.M.store_x2_last.
  case : (64 <= len{1}).
...
equiv pref_pavx2_cf_between_128_255 : 
  ChaCha20_pavx2_cf.M.chacha20_ref_loop ~ ChaCha20_pavx2_cf.M.chacha20_between_128_255 :
...
proof.
  proc => /=.
  transitivity{1}
    { (k, st_1) <@ ChaCha20_pavx2_cf.M.chacha20_body(st_1);
      (output, plain, len) <@ ChaCha20_pref.M.store (output, plain, len, k);
      (k, st_1) <@ ChaCha20_pavx2_cf.M.chacha20_body(st_1);
      (output, plain, len) <@ ChaCha20_pref.M.store (output, plain, len, k);
      ChaCha20_pavx2_cf.M.chacha20_ref_loop(output, plain, len, st_1);
    }
    (={output, plain, len, Glob.mem, st_1} /\ 128 <= len{2} <= 256 ==> ={Glob.mem})
    (={output, plain, len, Glob.mem, st_1} /\ 128 <= len{2} <= 256 ==> ={Glob.mem}).
  + smt(). + done.
  + inline ChaCha20_pavx2_cf.M.chacha20_ref_loop.
  ...
  + inline M.chacha20_body M.store_x2.
    ...
    by sim.
  transitivity{2}
    { ChaCha20_pavx2_cf.M.chacha20_less_than_128(output, plain, len, st_3); }
...
equiv pref_pavx2_cf_less_than_257 :  ChaCha20_pavx2_cf.M.chacha20_ref ~ ChaCha20_pavx2_cf.M.chacha20_less_than_257 :
...
module Body = {
  proc body (t: int * int * int * W32.t Array16.t, i:int) = {
    ...
    (k, st) <@ ChaCha20_pavx2_cf.M.chacha20_body(st);
    (output, plain, len) <@ ChaCha20_pref.M.store (output, plain, len, k);
...
equiv pref_pavx2_cf_more_than_256 : ChaCha20_pavx2_cf.M.chacha20_ref ~ ChaCha20_pavx2_cf.M.chacha20_more_than_256 :
...
proof.
  ...
  + smt(). + done.
  + inline ChaCha20_pavx2_cf.M.chacha20_ref_loop ILoop(Body).loop1 Body.body.
  ...
  + while ( #[/:-1]post /\ (0 <= i <= n){1}); last by auto => /#.
    inline [-tuple] M.body_x8 ChaCha20_pavx2_cf.M.store_x8 Body.body.
    swap{2} [44..46] -43.
  ...
  transitivity * {2} {
    if (256 <= len) {
      ...
      st_1 <@ ChaCha20_pref.M.increment_counter (st_1);
      (output, plain, len) <@  ChaCha20_pavx2_cf.M.store_x4(output, plain, len, k_1, k_2, k_3, k_4);
    }
    if (128 <= len) {
      ChaCha20_pavx2_cf.M.chacha20_between_128_255(output, plain, len, st_1);
    } else {
      ChaCha20_pavx2_cf.M.chacha20_less_than_128 (output, plain, len, st_1);
    }
...
equiv pref_pavx2_cf_chacha20 : ChaCha20_pref.M.chacha20_ref ~ ChaCha20_pavx2_cf.M.chacha20_avx2 :
