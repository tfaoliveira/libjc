...
module M = {
  ...
  proc init_x2 (key:W64.t, nonce:W64.t, counter:W32.t) : W256.t Array4.t = {
    ...
    st <- witness;
    nc <- g_p0;
    nc <- x86_VPINSR_4u32 nc counter (W8.of_int 0);
    nc <- x86_VPINSR_4u32 nc
    (loadW32 Glob.mem (W64.to_uint (nonce + (W64.of_int 0)))) (W8.of_int 1);
    nc <- x86_VPINSR_2u64 nc
    (loadW64 Glob.mem (W64.to_uint (nonce + (W64.of_int 4)))) (W8.of_int 1);
    s_nc <- nc;
    st.[0] <- x86_VPBROADCASTI_2u128 g_sigma;
    st.[1] <-
    x86_VPBROADCASTI_2u128 (loadW128 Glob.mem (W64.to_uint (key + (W64.of_int 0))));
    st.[2] <-
    x86_VPBROADCASTI_2u128 (loadW128 Glob.mem (W64.to_uint (key + (W64.of_int 16))));
    st.[3] <- x86_VPBROADCASTI_2u128 s_nc;
  ...
  proc init_x8 (key:W64.t, nonce:W64.t, counter:W32.t) : W256.t Array16.t = {
    ...
    while (i < 8) {
      st.[(i + 4)] <-
      x86_VPBROADCAST_8u32 (loadW32 Glob.mem (W64.to_uint (key + (W64.of_int (i * 4)))));
      i <- i + 1;
    }
    ...
    while (i < 3) {
      st.[(i + 13)] <-
      x86_VPBROADCAST_8u32 (loadW32 Glob.mem (W64.to_uint (nonce + (W64.of_int (i * 4)))));
      i <- i + 1;
    }
    st_ <- st;
    return (st_);
  ...
  proc perm_x2 (k:W256.t Array4.t) : W256.t Array4.t = {
    ...
    pk <- witness;
    pk.[0] <- x86_VPERM2I128 k.[0] k.[1] (W8.of_int (2 %% 2^4 + 2^4 * 0));
    pk.[1] <- x86_VPERM2I128 k.[2] k.[3] (W8.of_int (2 %% 2^4 + 2^4 * 0));
    pk.[2] <- x86_VPERM2I128 k.[0] k.[1] (W8.of_int (3 %% 2^4 + 2^4 * 1));
    pk.[3] <- x86_VPERM2I128 k.[2] k.[3] (W8.of_int (3 %% 2^4 + 2^4 * 1));
    return (pk);
  }
  ...
  proc store_last (output:W64.t, plain:W64.t, len:W32.t, k:W256.t Array2.t) : unit = {
    ...
    r1 <- x86_VEXTRACTI128 r0 (W8.of_int 0);
    if (((W32.of_int 16) \ule len)) {
      ...
      r1 <- x86_VEXTRACTI128 r0 (W8.of_int 1);
    } else {
  ...
  proc sub_rotate (t:W256.t Array8.t) : W256.t Array8.t = {
    ...
    i <- 0;
    while (i < 4) {
      t.[i] <- x86_VPERM2I128 x.[((2 * i) + 0)] x.[((2 * i) + 1)]
      (W8.of_int (2 %% 2^4 + 2^4 * 0));
      t.[(i + 4)] <- x86_VPERM2I128 x.[((2 * i) + 0)] x.[((2 * i) + 1)]
      (W8.of_int (3 %% 2^4 + 2^4 * 1));
      i <- i + 1;
    }
    return (t);
  }
  ...
  proc shuffle_state (k:W256.t Array4.t) : W256.t Array4.t = {
    ...
    k.[1] <- x86_VPSHUFD_256 k.[1]
    (W8.of_int (0 %% 2^2 + 2^2 * (3 %% 2^2 + 2^2 * (2 %% 2^2 + 2^2 * 1))));
    k.[2] <- x86_VPSHUFD_256 k.[2]
    (W8.of_int (1 %% 2^2 + 2^2 * (0 %% 2^2 + 2^2 * (3 %% 2^2 + 2^2 * 2))));
    k.[3] <- x86_VPSHUFD_256 k.[3]
    (W8.of_int (2 %% 2^2 + 2^2 * (1 %% 2^2 + 2^2 * (0 %% 2^2 + 2^2 * 3))));
    return (k);
  }
  ...
  proc column_round_x8 (k:W256.t Array16.t, k15:W256.t, k14:W256.t,
  ...
  proc diagonal_round_x8 (k:W256.t Array16.t, k15:W256.t, k14:W256.t,
                          s_r16:W256.t, s_r8:W256.t) : W256.t Array16.t *
                                                       W256.t = {
  ...
  proc rounds_x8 (k:W256.t Array16.t, s_r16:W256.t, s_r8:W256.t) : W256.t Array16.t = {
    ...
    while ((c \ult (W64.of_int 10))) {
      (k, k14) <@ column_round_x8 (k, k15, k14, s_r16, s_r8);
      (k, k15) <@ diagonal_round_x8 (k, k15, k14, s_r16, s_r8);
      c <- (c + (W64.of_int 1));
    }
    k.[15] <- k15;
    return (k);
