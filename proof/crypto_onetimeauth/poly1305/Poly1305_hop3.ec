...
module Mhop3 = {
  ...
  proc load_1x_Rep5(m:W64.t) : Rep5 = {
    ...
    var x : Rep5;
    t.[0] <- loadW64 Glob.mem (to_uint m);
  ...
  proc poly1305_long (out:W64.t, in_0:W64.t, inlen:W64.t, k:W64.t) : unit = {
    ...
    k <- (k + (W64.of_int 16));
    Rep3Impl.finish(out, in_0, inlen, k, h, r);
    return ();
 }
...
equiv update_eq :
 Mhop2.poly1305_update ~ Mhop3.Rep3Impl.update:
   ...
   r{1} = repres3r r{2} /\ Rep3r_ok r{2} /\ good_ptr in_0{2} (to_uint inlen{2})
 ==>
...
seq 2 1: (#[/1:5,6:]pre /\ ubW64 6 h{2}.[2]).
 ...
 call{2} (load_add_spec mem hh inp); wp; skip; progress.
  move: H2 H3; rewrite uleE; smt().
 by rewrite H7.
wp; exists* h{2}, r{2}; elim* => hh rr.
call{2} (Rep3Limb.mulmod_spec hh rr); skip; progress.
 smt().
...
equiv finish_eq :
 Mhop2.poly1305_finish ~ Mhop3.Rep3Impl.finish:
   ={Glob.mem, in_0, inlen, k, out} /\ h{1} = repres3 h{2} /\ ubW64 4 h{2}.[2] /\
   r{1} = repres3r r{2} /\ Rep3r_ok r{2} /\ to_uint inlen{2} < 16 /\
   good_ptr in_0{2} (to_uint inlen{2}) /\ good_ptr k{2} 16 /\ good_ptr out{2} 16
 ==>
   ={Glob.mem}.
proof.
proc => /=.
seq 2 4: (#pre /\ h_int{1} = valRep2 h2{2}).
 seq 1 3: (#pre).
  sp 0 2; if => //=.
  seq 2 1: (#[/1:9,10:]pre /\ ubW64 6 h{2}.[2]).
   exists* Glob.mem{2}, h{2}, in_0{2}, inlen{2}; elim* => mem hh inp iinlen.
   call{2} (load_last_add_spec mem hh inp iinlen); wp; skip; progress; smt().
  wp; exists* h{2}, r{2}; elim* => hh rr.
  call{2} (Rep3Limb.mulmod_spec hh rr); skip; progress; smt().
 exists* h{2}; elim* => hh.
 by call{2} (freeze_spec hh); wp; skip; progress; smt().
seq 1 1: (#pre /\ s{1}=valRep2 s{2}).
 exists* Glob.mem{2}, k{2}; elim* => mem kk.
 wp; call{2} (load2_spec mem kk); skip; progress; smt().
seq 1 1: (#pre).
 exists* h2{2}, s{2}; elim* => hh ss.
 wp; call{2} (add2_spec hh ss); skip; progress; smt().
exists* Glob.mem{2}, h2{2}, out{2}; elim* => mem hh oout.
by wp; call{2} (Rep3Limb.store2_spec mem oout hh); skip; progress.
qed.

equiv poly1305_short_eq :
 Mhop2.poly1305_short ~ Mhop3.Rep3Impl.poly1305:
   ={Glob.mem, in_0, inlen, k, out} /\
   inv_ptr k{2} in_0{2} inlen{2} out{2}
 ==>
   ={Glob.mem}.
proof.
proc => /=.
...
unroll for {2} 7.
simplify.
wp; call{2} (clamp_spec mem kk); wp; skip; rewrite /inv_ptr; progress.
+ smt().
+ by rewrite repres3E.
+ smt().
...
lemma load_1x_Rep5_spec_h mem ptr: 
  hoare [ Mhop3.load_1x_Rep5 :
            m = ptr /\ Glob.mem = mem /\ good_ptr ptr 16
          ==>
            bRep5 26 res /\ 
...
proof.
proc.
sp; exists* t; elim* => tt wit.
call (unpack2_bit128_spec_h tt).
skip => ?[<-[->[->[->?]]]]; split; first done.
move=> X ?[??]; clear X; split; first done.
split; last done.
...
lemma load4x_spec_h mem ptr: 
  hoare [ Mhop3.load_4x_Rep5 :
           m = ptr /\ Glob.mem = mem /\ good_ptr ptr 64
          ==>
           bRep5 26 res.`1 /\
           bRep5 26 res.`2 /\
           bRep5 26 res.`3 /\
...
equiv hop3eq : 
    Mhop2.poly1305 ~ Mhop3.poly1305 : 
      ={Glob.mem, out, in_0, inlen, k} /\ inv_ptr k{2} in_0{2} inlen{2} out{2}
    ==>
      ={Glob.mem}.
proof.
proc; if => //=; first by call poly1305_short_eq.
...
wp; seq 13 11: (256 < to_uint inlen0{2} /\ ={Glob.mem, out0, in_00, inlen0} /\
          good_ptr k0{2} 32 /\ good_ptr in_00{2} (to_uint inlen0{2} - 64) /\
          good_ptr out0{2} 16 /\
